...
function PanelDeExportacion({ rifa, ventasFiltradas = [], graficoRef }) {
    ...
    const generarExcel = () => {
        ...
        // =================================================================================================
        // INICIO DE LA CORRECIÓN EXCEL: Se trunca la lista de números si excede el límite de caracteres
        // =================================================================================================
        const participantesData = ventasFiltradas.map(v => {
            const numerosTexto = v.numeros.map(n => formatTicketNumber(n, rifa.boletos)).join(', ');
            // Límite de seguridad de Excel es 32767. Dejamos un margen.
            const numerosDisplay = numerosTexto.length > 32000 ? numerosTexto.substring(0, 32000) + `... (${v.numeros.length - numerosTexto.substring(0, 32000).split(',').length} más)` : numerosTexto;
