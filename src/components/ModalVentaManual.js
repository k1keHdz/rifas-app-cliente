...
function ModalVentaManual({ rifa, onClose }) {
  ...
  const generarMensajeWhatsApp = (venta) => {
    const boletosTexto = venta.numeros.map(n => String(n).padStart(paddingLength, '0')).join(', ');
      ...
      <div className="bg-white rounded-xl shadow-2xl p-4 sm:p-6 max-w-3xl w-full max-h-[95vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        {step === 1 && (
          <form onSubmit={handleSubmit} className="flex flex-col h-full overflow-hidden">
            ...
            <div className="mb-4 p-3 border rounded-lg bg-gray-50 flex-shrink-0 space-y-4">
              <div>
            ...
            {boletosSeleccionados.length > 0 && (
              <div className="mb-2 p-3 border rounded-lg bg-gray-50 flex-shrink-0">
                ...
                <div className="flex flex-wrap gap-1 max-h-20 overflow-y-auto">
                  {boletosSeleccionados.sort((a,b) => a-b).map(n => <span key={n} className="bg-green-100 text-green-800 px-2 py-0.5 rounded-full font-mono text-xs cursor-pointer" onClick={() => toggleBoleto(n)} title="Clic para quitar">{String(n).padStart(paddingLength, '0')}</span>)}
                </div>
