...
// ===============================================================================================
// INICIO DE LA MODIFICACIÓN: Componentes rediseñados para usar variables de theme.css
// ===============================================================================================

// Tarjeta de Sorteo que se adapta al tema
const RifaCard = ({ rifa, isFinished = false, onShowResults }) => {
    ...
    const CardContent = () => (
        <div className="relative group overflow-hidden rounded-xl h-full flex flex-col card">
            {/* La imagen y su superposición oscura se mantienen como parte del diseño estructural */}
            <img
                src={rifa.imagenes?.[0] || `https://placehold.co/600x400/171717/ffffff?text=Sorteo`}
                alt={rifa.nombre}
                className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 ease-in-out group-hover:scale-110"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent"></div>
            
            {/* Contenido de texto que usa colores del tema */}
            <div className="relative p-5 flex flex-col h-full text-white">
                ...
                <div className="mt-auto">
                    ...
                    <div className="mt-6">
                         {isFinished ? (
                            <button
                                onClick={() => onShowResults(rifa)}
                                className="w-full text-center font-bold py-3 px-5 rounded-lg transition-all text-sm uppercase btn-secondary"
                            >
                                Ver Resultados
                            </button>
                        ) : (
    ...
    return (
        <div className="h-full">
            {isFinished ? (
                ...
                <Link to={`/rifa/${rifa.id}`} className="block h-full">
                    <CardContent />
                </Link>
            )}
        </div>
...
// Hero Section que se adapta al tema
const HeroSection = () => (
...
// Sección de Pasos que se adapta al tema
const PasosSection = () => (
    <div className="bg-background-light py-20 sm:py-28">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="mt-16 grid gap-10 md:grid-cols-2 lg:grid-cols-4">
                <div className="text-center">
                    <div className="flex items-center justify-center h-16 w-16 mx-auto bg-background-dark border-2 border-accent-primary/50 text-accent-primary rounded-2xl">
                        <FaTicketAlt size={30} />
                    </div>
    ...
    const handleVerResultados = async (rifa) => { 
        ...
        try {  
            ...
            if (!querySnapshot.empty) {  
                setModalData({ winner: querySnapshot.docs[0].data(), totalBoletos: rifa.boletos });  
            } else {  
                setModalData(null);  
            }  
        } catch (error) {  
    ...
    return (  
        <div className="bg-background-dark">
            ...
            <main id="sorteos-activos" className="py-16 sm:py-20 bg-background-dark">  
                {cargando ? (  
                    <div className="text-center py-20 text-text-subtle">Cargando...</div>  
                    ...
                    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        {totalPaginasActivas > 1 && (
                            <div className="mt-12 flex justify-center items-center gap-4">
                                <button 
                                    onClick={() => setPaginaActivas(p => p - 1)} 
                                    disabled={paginaActivas === 1}
                                    className="px-4 py-2 bg-background-light text-text-primary font-semibold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-opacity-90 transition-colors"
                                >
                                    Anterior
                                </button>
                                <span className="font-medium text-text-subtle">
                                    Página {paginaActivas} de {totalPaginasActivas}
                                </span>
                                <button 
                                    onClick={() => setPaginaActivas(p => p + 1)} 
                                    disabled={paginaActivas === totalPaginasActivas}
                                    className="px-4 py-2 bg-background-light text-text-primary font-semibold rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-opacity-90 transition-colors"
                                >
                                    Siguiente
                                </button>
