...
function GestionarRifasPage() {
  ...
  return (
    <div className="bg-background-dark text-text-light p-4 max-w-7xl mx-auto relative min-h-screen">
      ...
      <div className="border-b border-border-color mb-6 mt-4">
        <nav className="-mb-px flex space-x-6" aria-label="Tabs">
          <button onClick={() => handleTabChange('lista')} className={`flex items-center whitespace-nowrap py-3 px-4 border-b-2 font-medium text-sm transition-colors ${ activeTab === 'lista' ? 'border-accent-start text-accent-start' : 'border-transparent text-text-subtle hover:text-text-light hover:border-border-color' }`}><ListIcon /> Listado de Sorteos</button>
          {isFormVisible && (
            <button onClick={() => handleTabChange('editor')} className={`flex items-center whitespace-nowrap py-3 px-4 border-b-2 font-medium text-sm transition-colors ${ activeTab === 'editor' ? 'border-accent-start text-accent-start' : 'border-transparent text-text-subtle hover:text-text-light hover:border-border-color' }`}><EditIcon /> Editor de Sorteo</button>
          )}
        </nav>
      </div>
