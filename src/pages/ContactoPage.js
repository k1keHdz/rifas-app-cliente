...
const SocialIcon = ({ href, title, children }) => (
    <a href={href} target="_blank" rel="noopener noreferrer" title={title} className="text-text-subtle hover:text-white transition-all duration-300 transform hover:scale-110">
        {children}
    </a>
);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="max-w-3xl mx-auto">
                    <div className="text-center">
                        <h1 className="text-4xl font-extrabold text-text-light tracking-tight sm:text-5xl">Contáctanos</h1>
                    ...
                    <div className="mt-12 bg-background-light border border-border-color p-6 sm:p-8 rounded-2xl shadow-lg">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            <div>
                               <label htmlFor="from_name" className="block text-sm font-medium text-text-light">Tu Nombre</label>
                               <div className="mt-1">
                                   <input type="text" name="from_name" id="from_name" value={formData.from_name} onChange={handleChange} required className="w-full bg-background-dark text-text-light border-border-color rounded-md shadow-sm focus:ring-accent-start focus:border-accent-start" />
                               </div>
                            </div>
                            <div>
                               <label htmlFor="from_email" className="block text-sm font-medium text-text-light">Tu Correo Electrónico</label>
                               <div className="mt-1">
                                   <input type="email" name="from_email" id="from_email" value={formData.from_email} onChange={handleChange} required className="w-full bg-background-dark text-text-light border-border-color rounded-md shadow-sm focus:ring-accent-start focus:border-accent-start" />
                               </div>
                            </div>
                            <div>
                               <label htmlFor="message" className="block text-sm font-medium text-text-light">Mensaje</label>
                               <div className="mt-1">
                                   <textarea id="message" name="message" rows={4} value={formData.message} onChange={handleChange} required className="w-full bg-background-dark text-text-light border-border-color rounded-md shadow-sm focus:ring-accent-start focus:border-accent-start"></textarea>
                               </div>
                            </div>
                            ...
                            <div>
                               <button type="submit" disabled={isSubmitting} className="w-full flex justify-center py-3 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-gradient-to-r from-accent-start to-accent-end hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent-start disabled:opacity-50">
                    ...
                    <div className="mt-16 text-center">
                        <h3 className="text-2xl font-bold text-text-light">Síguenos en Nuestras Redes</h3>
