...
/**
 * Componente que gestiona las rutas.
 * Se renderiza DESPUÉS de que ConfigContext esté disponible.
 */
function AppRoutes() {
  const { config, cargandoConfig } = useConfig();

  // Muestra un estado de carga mientras la configuración se obtiene de Firebase.
  if (cargandoConfig) {
...
/**
 * Componente principal que envuelve la aplicación con los contextos y el router.
 * La clave es que la lógica de renderizado (Navbar, AppRoutes, Footer) está DENTRO
 * de los providers, rompiendo el ciclo de dependencias.
 */
function App() {
  return (
    <AuthProvider>
